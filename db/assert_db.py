...
def request_db(select_query, base_url):
    request_bd = RequestDB()
    result = request_bd.request(select_query, base_url)
    return result
...
class GetResultDB:
    ...
    def res_value_create_account(self, source, compiler, base_url, application_id):
        ...
        result = request_db(select_db.select_query, base_url=base_url)
        pattern = compiler
        matches = []
        # Поиска совпадений в каждой строке результата
        for row in result:
...
class AssertDB:
    ...
    def assert_cft_updateaccount(self, base_url, application_id):
        ...
        self.transfer_account = get_result_db.res_value_create_account(source='CFT_CREATE_ACCOUNT',
                                                                       compiler=compiler,
                                                                       base_url=base_url,
                                                                       application_id=application_id)
        ...
        with allure.step('Проверка, что в XBPM_CFT_UPDATEACCOUNT передаётся верный transfer_account'):
            assert self.transfer_account == value_receiver_acc_updateaccount, \
    ...
    def assert_transfer_installment(self, base_url, application_id):
        get_result_db = GetResultDB()
        compiler = re.compile(
            r'<ns2:DOC_TYPE>TRANSFER_INSTALLMENT</ns2:DOC_TYPE>.*?<ns2:SENDER_ACC>(.*?)</ns2:SENDER_ACC>', re.DOTALL)
        ...
        with allure.step(
                'Проверка, что в XBPM_CFT_TRANSFERCARD по услуге TRANSFER_INSTALLMENT передался верный SENDER_ACC (account_number)'):
        ...
        compiler = re.compile(
            r'<ns2:DOC_TYPE>TRANSFER_INSTALLMENT</ns2:DOC_TYPE>.*?<ns2:RECEIVER_ACC>(.*?)</ns2:RECEIVER_ACC>',
            re.DOTALL)
        ...
        with allure.step(
                'Проверка, что в XBPM_CFT_TRANSFERCARD по услуге TRANSFER_INSTALLMENT передался верный RECEIVER_ACC'):
    ...
    def assert_payment_box(self, base_url, application_id):
        get_result_db = GetResultDB()
        compiler = re.compile(r'<ns2:DOC_TYPE>PAYMENT_BOX</ns2:DOC_TYPE>.*?<ns2:SENDER_ACC>(.*?)</ns2:SENDER_ACC>',
                              re.DOTALL)
        sender_acc_payment_box = get_result_db.external_source_request(source='XBPM_CFT_TRANSFERCARD',
                                                                       ...
                                                                       application_id=application_id)
        with allure.step(
                'Проверка, что в XBPM_CFT_TRANSFERCARD по услуге PAYMENT_BOX передался верный SENDER_ACC (transfer_account)'):
        ...
        compiler = re.compile(r'<ns2:DOC_TYPE>PAYMENT_BOX</ns2:DOC_TYPE>.*?<ns2:DESCRIPTION>(.*?)</ns2:DESCRIPTION>',
                              re.DOTALL)
        description_payment_box = get_result_db.external_source_request(source='XBPM_CFT_TRANSFERCARD',
                                                                        ...
                                                                        application_id=application_id)
        with allure.step(
                'Проверка, что в XBPM_CFT_TRANSFERCARD по услуге PAYMENT_BOX отправилось значение по двум коробкам'):
