    ...
    def go_application(self, number_app):
        ...
        while True:
            if self.element_is_not_clickable(*MyApplicationsLocators.BUTTON_APPLY, timeout=1):
                button_search_parameters = self.find_element(*MyApplicationsLocators.BUTTON_SEARCH_PARAMETERS)
                button_search_parameters.click()
                time.sleep(1)
            else:
                break

        input_number_req = self.find_element(*MyApplicationsLocators.INPUT_NUMBER_REQ)
        button_apply = self.find_element(*MyApplicationsLocators.BUTTON_APPLY)
